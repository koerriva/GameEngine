...
GLuint Star::GenerateStarTexture(const unsigned int size)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE8, size, size, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, pixels);
...
{
  return TransformToWorld(TransformToLocal(pos, true).GetNormalized(), false) * radius;
}
...
void Star::Render() const
{
  ...
  static const GLint uniformStarTex = glGetUniformLocation(ShaderManager::GetShaderProgramHandle(SUN), "starTex");
